...
#Adicionar postagens
    def adicionar_postagem(self, nome_objeto, localidade, data, telefone, usuario_logado, status):
        if not all([nome_objeto, localidade, data, telefone, usuario_logado]): #verifica se todos os dados foram preenchidos
            raise ValueError("Todos os campos devem ser preenchidos.")
        self.validar_data(data)
        nova_postagem = Objeto(nome_objeto=nome_objeto, data=data,localidade=localidade, telefone=telefone,user_id=usuario_logado.id, status=status)
...
#Lista de postagens cadastradas
    ...
    def minhas_postagens(self, user):
        ...
        for i, obj in enumerate(objetos, 1):
            ...
            print(f"   Localidade: {obj.localidade}, Data: {obj.data}, Telefone: {obj.telefone}")
        ...
        if not objetos:
            ...
            return
        escolha=int(input("Digite o ID do objeto que deseja alterar o status:"))
